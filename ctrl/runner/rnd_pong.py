...
def train(_model_name, _timestamp, _model_dir, _render):
    ...
    agent = RNDAgent(env, name=_model_name, timestamp=_timestamp, train_start=50000)
    global_step = 0
    size_of_window = 15
    scores = []
    mean_score = 0.0
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            # RND 학습 시에 매 step 마다 학습하는 경우 너무 오랜 시간과 랙 발생
...
def test(_model_name, _timestamp, _model_dir):
    ...
    agent = RNDAgent(wenv, name=_model_name, timestamp=_timestamp, epsilon=0.0)
